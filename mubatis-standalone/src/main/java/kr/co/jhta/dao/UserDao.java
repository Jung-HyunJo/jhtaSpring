...
public class UserDao {
	...
	public void deletedAllUser() {
		SqlSession session = MybatisUtil.getSqlSession();
		try {
			session.delete("deletedAllUser");
			session.commit();
		}finally {
			session.close();
		}
	...
	public void deleteUerById(String userId) {
		SqlSession session = MybatisUtil.getSqlSession();
		try {
			session.delete("deleteUerById", userId);
			session.commit();
		}finally {
			session.close();
		}
	...
	// select: 1행 N열, parameter:stringm reslut:User 객체, 최종값:User객체
